...
class data_convert:
    def __init__(self):
        ...
        self.parm_exp = [str(i) for i in range(0, 4)]  # 經驗值等級
        self.parm_job = [str(i) for i in range(1, 5)]  # 職務等級； 目前 1 ~ 4
        self.parm_cate = [str(i) for i in range(1, 200)] + [
            str(i) for i in range(300, 700)
    ...
    def fn_proj_eventbooks(
        ...
        try:
            ...
            if _project_name_ not in [
                i for i in self.parm_project
            ...
            for j in _devices_:  # 依照 device 進行區分
                ...
                ):  # 優先順序範圍需是"數字" 且最大值不大於需要派遣的category種類數
                    self.df_error_list = _events_[
