...
class data_convert():
    ...
    def __init__(self,para):
        ...
        self.parm_shift = list(self.df_parm["shift"].dropna().astype(int).astype(str)) # 班別:白班(0) 夜班(1)
        self.parm_project = list(self.df_parm["project"].dropna().astype(str)) # 專案別 ；MySQL中有出現 D5X、PEPPER 等命名方式；注意 D5X 不是 "D52","D53","D54" 共用的意思
        self.parm_process = list(self.df_parm["process"].dropna().astype(str)) # 製程段
        self.parm_device = list(self.df_parm["device"].dropna().astype(str)) # 機台名稱； 目前有 Device_1~13
        self.parm_exp = list(self.df_parm["exp"].dropna().astype(int).astype(str)) # 經驗值等級
        self.parm_job = list(self.df_parm["job"].dropna().astype(int).astype(str)) # 職務等級； 目前 1 ~ 4
        self.parm_cate = list(self.df_parm["category"].dropna().astype(int).astype(str)) # 需要指派員工的category ； 當前為 1~199,300~699
        self.parm_workshop = list(self.df_parm["workshop"].dropna().astype(str)) # 車間： 目前有 第九車間(9)，可為數字或是string，最後都會轉成string
        return
    #%%
    """
    車間員工資訊表
    ...
    def fn_factory_worker_info(self,raw_excel:bytes):
        ...
        try: 
            ...
            print("轉換完成")
            return self.df_factory_worker_info_convert,self.df_worker_info.sort_values(by="職務",ascending=0) # 資料表轉換完成，提供給server做後續匯入動作
...
#%%
    ...
    def fn_proj_eventbooks(self,filename:str, raw_excel:bytes): # 輸入資料路徑與名稱；須注意資料名稱格式
        ...
        try:             
            ...
            if _project_name_ not in self.parm_project: # 檢查資料表名稱 e.g. D5X device事件簿.xlsx 會取得 D5X
                raise Error_Project
            ...
            for j in tqdm(_devices_): # 依照 device 進行區分
                if ~self.df_proj_eventbooks[j].columns.isin(["Category","MESSAGE","优先顺序"]).all():# 檢查欄位名稱是否正確；確定匯入的資料正確
                ...
                if _events_[["MESSAGE","优先顺序"]].isnull().values.any(): # 檢查單一機台的資料中是否有空值
                ...
                if ~_events_["优先顺序"].apply(lambda y:isinstance(y,(float,int))).all():# 優先順序範圍需是"數字"
...
#%%
"""測試資料匯入"""
test_file_path_parainfo = "test_data/程式碼參數_公版.xlsx" # 參數資料路徑
# test_data_import = data_convert(test_file_path_parainfo) # 建立物件
