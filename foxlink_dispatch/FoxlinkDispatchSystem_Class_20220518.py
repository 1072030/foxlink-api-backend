...
class data_convert:
    def __init__(self):
        return
    #參考資料(自然排序):https://stackoverflow.com/questions/4836710/is-there-a-built-in-function-for-string-natural-sort
    ...
    def fn_factory_worker_info(self,raw_excel:bytes):
        ...
        try: 
            ...
            if ~self.df_worker_info.columns.isin(["班別","員工工號","員工名字","車間","職務","負責人"]).all(): # 檢查資料表內容，如果沒有此些值，合理判斷是上傳錯誤的內容，或是公版被修改
            ...
            if self.df_worker_info.isnull().values.any(): # 檢查是否有空值；可以避免空欄位
                self.df_error_list = self.df_worker_info[self.df_worker_info.isnull().values]
                raise Error_None
            ...
            for w in tqdm(range(len(self.df_worker_info))):# 員工數量
                for p in range(len(self.df_project_info.columns)): # 所有專案中機台數量
                    self.df_factory_worker_info_convert = pd.concat([self.df_factory_worker_info_convert,
                                                                     pd.DataFrame([{"worker_id":str(self.df_worker_info["員工工號"].iloc[w]), # str； 員工工號
            ...
            "找出各欄位參數資訊"
            # 參考資料(包資料)： https://stackoverflow.com/questions/16031056/how-to-form-tuple-column-from-two-columns-in-pandas
            parm = {"worker": list(zip(self.df_worker_info["員工工號"],self.df_worker_info["員工名字"])),
                    "workshop":sorted(list(set(self.df_factory_worker_info_convert["workshop"]))),
                    "project":sorted(list(set(self.df_factory_worker_info_convert["project"]))),
                    "process":self.natural_sort(list(set(self.df_factory_worker_info_convert["process"]))),
                    # "device":self.natural_sort(list(set(self.df_factory_worker_info_convert["device_name"]))),
                    "device":list(zip(self.df_project_info.loc["專案"], self.df_project_info.loc["自動機製程段"], self.df_project_info.loc["Device"], self.df_project_info.loc["設備中文名稱"])),
                    "shift":sorted(list(set(self.df_factory_worker_info_convert["shift"]))),
            ...
            self.df_factory_worker_info_parm = pd.concat([pd.Series(v, name=k) for k, v in parm.items()],axis = 1)
            
            print("轉換完成")
            # 資料表轉換完成，提供給server做後續匯入動作
            return {"result":self.df_factory_worker_info_convert, # 轉換完成的結果
                    "worker_info":self.df_worker_info.sort_values(by="職務",ascending=0), # 員工資訊
                    "parameter":self.df_factory_worker_info_parm} # 相關參數
...
#%%
    ...
    def fn_proj_eventbooks(self,filename:str, raw_excel:bytes): # 輸入資料路徑與名稱；須注意資料名稱格式
        ...
        print(filename+" 轉換中...")
        try:             
            _project_name_ = filename.split("Device")[0].strip(" ") # 抓取 project 檔案名稱；用 "Device" 去切，注意忽略"空格"
            ...
            for j in tqdm(_devices_): # 依照 device 進行區分
                ...
                if ~_events_.columns.isin(["Category","MESSAGE","优先顺序"]).all():# 檢查欄位名稱是否正確；確定匯入的資料正確
                    self.df_error_list = _events_.columns
                    value = set(_events_.columns).difference(["Category","MESSAGE","优先顺序"])
                    raise Error_FileContent
            ...
            parm = {"category":sorted(list(set(self.df_proj_eventbooks_convert[~self.df_proj_eventbooks_convert["优先顺序"].isna()]["Category"])))}
            self.df_proj_eventbooks_parm = pd.DataFrame(parm)
            
            print("轉換完成")
            # 資料表轉換完成，提供給server做後續匯入動作
            return {"result":self.df_proj_eventbooks_convert, # 轉換完成的結果
                    "parameter":self.df_proj_eventbooks_parm} # 相關參數
...
#%%    
    ...
    def fn_factorymap(self,raw_excel:bytes):# 輸入車間機台座標資料表，生成簡易移動距離矩陣
        ...
        try:
            ...
            if ~self.df_device_xy.columns.isin(["id","workshop","project","process","line","device_name","x_axis","y_axis","sop_link"]).all(): # 檢查欄位名稱是否正確；確定匯入的資料正確
                self.df_error_list = self.df_device_xy.columns
                raise Error_FileContent
            if self.df_device_xy[["id","x_axis","y_axis"]].isnull().values.any(): # done # 確認資料表"id","x_axis","y_axis"欄位有無空值；才可算移動距離
                self.df_error_list = self.df_device_xy[self.df_device_xy[["id","x_axis","y_axis"]].isnull().values]
                raise Error_None
            ...
            if _devices_.isnull().values.any(): # done # 確認所有機台相關欄位無空值
                ...
                raise Error_None  
            "找出各欄位參數資訊"
            parm = {"workshop":sorted(list(set(_devices_["workshop"]))),
                ...
                for j in range(i,len(self.df_device_xy)):
                    ...
                    if From_device["process"] == To_device["process"]: # 判斷兩device 是否屬於相同製程(process)
                        ...
                        if self.df_device_xy[self.df_device_xy["process"]==From_device["process"]]["y_axis"].between(min(From_device["y_axis"],To_device["y_axis"]),max(From_device["y_axis"],To_device["y_axis"]),inclusive="neither").any():
                            ...
                            fit_xy = self.df_device_xy[((self.df_device_xy[["project","process","line"]] == From_device[["project","process","line"]]).all(axis=1))] # 選出符合條件的座標資料
                            From_left = self.df_device_xy.iloc[fit_xy["x_axis"].idxmin()] # 左邊
                            From_right = self.df_device_xy.iloc[fit_xy["x_axis"].idxmax()] # 右邊
                            # 計算 Manhattan Distance；是由"起始device"到"左、右"的距離，再加上"左、右"到"終點"的距離
                            From_left_dis = distance.cityblock([From_device["x_axis"],From_device["y_axis"],From_left["x_axis"],From_left["y_axis"]],
                                                               [From_left["x_axis"],From_left["y_axis"],To_device["x_axis"],To_device["y_axis"]])
                            From_right_dis = distance.cityblock([From_device["x_axis"],From_device["y_axis"],From_right["x_axis"],From_right["y_axis"]],
                                                                [From_right["x_axis"],From_right["y_axis"],To_device["x_axis"],To_device["y_axis"]])
                            dis_cal = min(From_left_dis,From_right_dis) # 移動有左右兩種，取其中較短的距離
                        else:
                            dis_cal = distance.cityblock(From_device[["x_axis","y_axis"]],To_device[["x_axis","y_axis"]])
                    else:
                        dis_cal = distance.cityblock(From_device[["x_axis","y_axis"]],To_device[["x_axis","y_axis"]]) # 屬於不同製程，直接計算 Manhattan Distance
                    # "儲存移動距離"
            ...
            print("轉換完成")
             # 回傳計算完的機台間移動距離矩陣表
            return {"result":self.df_movingMatrix, # 轉換完成的結果
                    "parameter":self.df_factorymap_parm} # 相關參數
...
# test_eventbook = test_data_import.fn_proj_eventbooks(test_file_path_eventbook.split("/")[-1],test_file_path_eventbook)
...
# test_parm = test_data_import.fn_parm_update([test_workerinfo["parameter"],test_eventbook["parameter"],test_moving_matrix["parameter"]]) # 更新參數表
