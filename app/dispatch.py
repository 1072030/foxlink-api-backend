...
class data_convert:
    def __init__(self):
        """可控參數(parm)"""
        self.parm_cate = [i for i in range(1, 200)] + [
            i for i in range(300, 700)
        ]  # 需要指派員工的category ； 當前為 1~199,300~699
        self.parm_process = ["M1段", "M2段", "M3段"]  # 製程段
        self.parm_machine = [
            "Device_" + str(i) for i in range(1, 14)
    ...
    def fn_factory_worker_info(self, excel_file_path):
        ...
        self.df_factory_worker_info = pd.read_excel(
            excel_file_path, sheet_name=0, header=None
        )
        "抓取員工資訊；職位判斷，負責人關係..."
        ...
        for w in range(len(self.df_worker_info)):  # 員工數量
            for p in range(len(self.df_project_info.columns)):  # 所有專案中機台數量
                ...
                self.df_factory_worker_info_convert = self.df_factory_worker_info_convert.append(
                    {
                        "worker_id": str(
                            self.df_worker_info["員工編號"].iloc[w]
                        ),  # str； 員工工號
                        "worker_name": str(
    ...
    def fn_proj_eventbooks(
        ...
        self.df_proj_eventbooks_convert = self.df_proj_eventbooks_convert[
            self.df_proj_eventbooks_convert["Category"].isin(self.parm_cate)
    ...
    def fn_factorymap(self, excel_file_path):  # 輸入車間機台座標資料表，生成簡易移動距離矩陣
        ...
        self.df_movingMatrix = pd.DataFrame(
            index=self.df_deivce_xy["id"], columns=self.df_deivce_xy["id"]
        ...
        for i in range(len(self.df_deivce_xy)):
            ...
            for j in range(i, len(self.df_deivce_xy)):
                ...
                if (
                    ...
                    if (
                        self.df_deivce_xy[
                            self.df_deivce_xy["process"] == From_device["process"]
                        ]["y_axis"]
                        .between(
                        ...
                        From_left = self.df_deivce_xy.iloc[
                            self.df_deivce_xy[
                                (self.df_deivce_xy["process"] == From_device["process"])
                                & (
                                    self.df_deivce_xy["project"]
                                    == From_device["project"]
                                )
                            ]["x_axis"].idxmin()
                        ]
                        From_right = self.df_deivce_xy.iloc[
                            self.df_deivce_xy[
                                (self.df_deivce_xy["process"] == From_device["process"])
                                & (
                                    self.df_deivce_xy["project"]
