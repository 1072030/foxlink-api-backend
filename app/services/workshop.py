...
async def get_all_devices_status(workshop_name: str, has_rescue=False):
    ...
    if has_rescue:
        # get all devices, rescue stations also included
        devices = await Device.objects.filter(workshop=workshop).all()
    else:
        devices = await Device.objects.filter(workshop=workshop, is_rescue=has_rescue).all()
